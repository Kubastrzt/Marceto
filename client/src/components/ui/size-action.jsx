...
export const SizeActions = ({data})=>{
    ...
    const onDelete = async ()=>{
        try {
            setLoading(true)
            await toast.promise(axios.delete(`http://localhost:3001/api/${params.sid}/${userId}/sizes/${data.id}`),{
                pending: 'Usuwanie...',
                success: 'Rozmiar został usunięty 👌',
    ...
    return(
        <>
            ...
            <DropdownMenuContent align="end" className='bg-gray-700 text-white'>
                ...
                <DropdownMenuItem className='cursor-pointer' onClick={()=>navigate(`/sizes/${params.sid}/${data.id}`)}>
